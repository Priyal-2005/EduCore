...
async function main() {
    ...
    const adminUser = await prisma.user.create({
        data: { email: 'admin@educore.com', password: passwordHash, role: client_1.UserRole.ADMIN },
    });
    const teacherUser1 = await prisma.user.create({
        data: { email: 'john.smith@educore.com', password: passwordHash, role: client_1.UserRole.TEACHER },
    });
    const teacherUser2 = await prisma.user.create({
        data: { email: 'sarah.jones@educore.com', password: passwordHash, role: client_1.UserRole.TEACHER },
    });
    const teacherUser3 = await prisma.user.create({
        data: { email: 'michael.brown@educore.com', password: passwordHash, role: client_1.UserRole.TEACHER },
    });
    const parentUser1 = await prisma.user.create({
        data: { email: 'parent.doe@gmail.com', password: passwordHash, role: client_1.UserRole.PARENT },
    });
    const parentUser2 = await prisma.user.create({
        data: { email: 'parent.wilson@gmail.com', password: passwordHash, role: client_1.UserRole.PARENT },
    });
    const studentUser1 = await prisma.user.create({
        data: { email: 'alice.doe@student.educore.com', password: passwordHash, role: client_1.UserRole.STUDENT },
    });
    const studentUser2 = await prisma.user.create({
        data: { email: 'bob.doe@student.educore.com', password: passwordHash, role: client_1.UserRole.STUDENT },
    });
    const studentUser3 = await prisma.user.create({
        data: { email: 'charlie.wilson@student.educore.com', password: passwordHash, role: client_1.UserRole.STUDENT },
    });
    const studentUser4 = await prisma.user.create({
        data: { email: 'diana.patel@student.educore.com', password: passwordHash, role: client_1.UserRole.STUDENT },
    });
    const studentUser5 = await prisma.user.create({
        data: { email: 'ethan.lee@student.educore.com', password: passwordHash, role: client_1.UserRole.STUDENT },
    });
    // ━━━ Parents ━━━
    ...
    await prisma.attendance.createMany({
        data: [
            { studentId: student1.id, date: yesterday, status: client_1.AttendanceStatus.PRESENT },
            { studentId: student2.id, date: yesterday, status: client_1.AttendanceStatus.PRESENT },
            { studentId: student3.id, date: yesterday, status: client_1.AttendanceStatus.ABSENT },
            { studentId: student4.id, date: yesterday, status: client_1.AttendanceStatus.LATE },
            { studentId: student5.id, date: yesterday, status: client_1.AttendanceStatus.PRESENT },
        ],
    });
